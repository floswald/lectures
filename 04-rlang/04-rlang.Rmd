---
title: "ScPo Intro to Programming"
# subtitle: "<html><div style='float:left'></div><hr color='#EB811B' size=1px width=796px></html>"
subtitle: "Lecture 6: R language basics"
author: "Grant McDermott (U of Oregon) + Florian Oswald"
date: "SciencesPo Paris | [Intro to Programming](https://floswald.github.io/ScPoProgramming/)" #"`r format(Sys.time(), '%d %B %Y')`"
output:
...
# This beautiful set of slides is largely the work of [Grant McDermott](https://grantmcdermott.com/)
...
```{r, echo=F}
# murders <- read.csv("https://floswald.github.io/ScPoProgramming/data/brexit.csv")
```

1. Find out (using `help()` or google) how to import a .csv file. Do NOT use the "Import Dataset" button, nor install a package.

1. Download the csv file [brexit.csv](https://floswald.github.io/ScPoProgramming/data/brexit.csv) and create a new object called `brexit` (Hint: objects are created using `=` or `<-`).
...
1. Create a new object `brexit_3` which only contains the columns `startdate` and `samplesize`. (Recall that `c` creates vectors.)

1. Create a `total_remains` variable equal to the total number of remain votes in each poll;
    ```{r,eval = FALSE}
    brexit$total_remains = (brexit$remain * brexit$samplesize)
    ```
