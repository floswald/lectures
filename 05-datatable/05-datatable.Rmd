...
subtitle: "`R`: more data wrangling with `data.table`"
...
output:
  xaringan::moon_reader:
    css: [default, metropolis, metropolis-fonts, "libs/cols.css", "styles.css"] 
    lib_dir: libs
    nature:
...
* Mention `%like%` or `grepl` for some `names`
--
...
```{r collapse_comp_key,  echo=FALSE, message=FALSE}
storms_dt_key = as.data.table(storms, key = c("name", "year", "month", "day"))
collapse_dt_key = function() {
  storms_dt_key[, .(wind = mean(wind), pressure = mean(pressure), category = first(category)),
            by = .(name, year, month, day)]
...
mbx_join = round((mb_join_dt[expr=="merge_dt()", median(time)] /
                    mb_join_dt[expr=="merge_dt_key()", median(time)]))
...
- https://h2oai.github.io/db-benchmark/ The ultimate benchmark between different data-wrangling solutions
...
# Next lecture: Building R Packages
<html><div style='float:left'></div><hr color='#EB811B' size=1px width=796px></html>
